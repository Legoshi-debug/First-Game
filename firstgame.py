...
async def main():
    ...
    def load_level(index):
        nonlocal platforms, last_platform, goal
        platforms = levels[index]
        last_platform = last_platform_rect(platforms)
        goal = pygame.Rect(last_platform.x + 50, last_platform.y - 80, 120, 100)

    def reset_level():
        nonlocal player_x, player_y, player_vel_y, jumps_left, leg_segments, on_ground, camera_x, collapsed
        player_x, player_y = start_x, start_y
        player_vel_y = 0
        leg_segments = 8
        jumps_left = leg_segments * 2
    ...
    while running:
        ...
        if game_state==START:
            ...
            if current_level_index == len(levels)-1:
                draw_text_center("The END", small_font, WHITE, WIDTH//2, HEIGHT//2+40)
            else:
